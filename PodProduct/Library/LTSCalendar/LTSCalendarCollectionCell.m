...
- (void)commonInit{
    ...
    holidayLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, 10, 12)];
    holidayLabel.font = _holidayLabelFont;
    holidayLabel.textColor = _holidayLabelTextColor;
    holidayLabel.text = @"休";
...
- (void)setItem:(LTSCalendarDayItem *)item{
    ...
    if (item.holidayType==HolidayTypeNone) {
        holidayLabel.hidden = YES;
    }else{
        holidayLabel.hidden = NO;
        if (item.holidayType==HolidayTypeRest) {
            holidayLabel.text = @"休";
        }else{
            holidayLabel.text = @"班";
        }
...
//    circleView.frame = CGRectMake(0, 0, sizeCircle, sizeCircle);
    circleView.frame = CGRectMake(0, 0, 38, 44);
    ...
    dotView.frame = CGRectMake(0, CGRectGetMaxY(circleView.frame)-sizeDot*1.5, sizeDot, sizeDot);
    dotView.center = CGPointMake(self.frame.size.width / 2., dotView.center.y );
    
    dotView.layer.cornerRadius = sizeDot / 2.;
    
    holidayLabel.right = circleView.right;
...
- (void)setSelected:(BOOL)isSelected animated:(BOOL)animated{
    ...
    if(isSelected){
        ...
        if (self.item.showEventDot) {
            dotView.color = [UIColor whiteColor];
        }
        if (self.item.holidayType!=HolidayTypeNone) {
            holidayLabel.textColor = [UIColor whiteColor];
        }
        ...
        if (self.item.showEventDot) {
            dotView.color = _holidayLabelTextColor;
        }
        if (self.item.holidayType==HolidayTypeRest) {
            holidayLabel.textColor = _holidayLabelTextColor;
        }
        else if(self.item.holidayType==HolidayTypeWork)
        {
            holidayLabel.textColor = _holidayLabelTextColorWork;
        }
        if ([self isToday]){
...
//            lunarTextLabel.textColor = [LTSCalendarAppearance share].lunarDayTextColor;
            ...
            circleView.layer.borderColor = [UIColor clearColor].CGColor;
            textLabel.textColor = _holidayLabelTextColor;
            lunarTextLabel.textColor = _holidayLabelTextColor;
