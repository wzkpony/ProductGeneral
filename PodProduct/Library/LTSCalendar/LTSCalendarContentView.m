...
//    self.backgroundColor = [UIColor orangeColor];
    ...
    UIView *line = [[UIView alloc]initWithFrame:CGRectMake(0, 0, 1, self.height-10)];
...
- (void)getDateDatas{
    ...
    [self.daysInMonth enumerateObjectsUsingBlock:^(NSArray *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [obj enumerateObjectsUsingBlock:^(LTSCalendarDayItem *item, NSUInteger idx, BOOL * _Nonnull sstop) {
            if (item.isSelected) {
                NSLog(@"select = %@ index = %ld",item.date,idx);
                [[NSNotificationCenter defaultCenter]postNotificationName:@"refreshWeekNumNoti" object:@(ceil((idx+1)/7.))];
                *sstop = YES;
                *stop = YES;
            }else{
                NSLog(@"nonononoselect = %@",item.date);
            }
        }];
    }];
...
- (NSArray *)getDaysOfWeek:(NSDate *)date
{
    ...
    for (NSInteger i=0; i<7; i++) {
        ...
        if (self.eventSource && [self.eventSource respondsToSelector:@selector(calendarIsHolidays:)]) {
            item.holidayType = [self.eventSource calendarIsHolidays:currentDate];
        }
        NSDateComponents *dayComponent = [NSDateComponents new];
        dayComponent.day = 1;
