...
@interface PYSearchViewController () <UISearchBarDelegate, UITableViewDelegate, UITableViewDataSource, PYSearchSuggestionViewDataSource,UITextFieldDelegate>

@property (nonatomic, strong) UITextField *searchBar;//新增
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (self.lastSearchStr) {
        _searchBar.text = self.lastSearchStr;
    }
...
//        UITableView *baseSearchTableView = [[UITableView alloc] initWithFrame:self.view.bounds style:UITableViewStyleGrouped];
        UITableView *baseSearchTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, kNavHeight, KWIDTH, KHEIGHT-kNavHeight) style:UITableViewStyleGrouped];
...
//        searchSuggestionVC.view.frame = CGRectMake(0, CGRectGetMaxY(self.navigationController.navigationBar.frame), PYScreenW, PYScreenH);
        searchSuggestionVC.view.frame = CGRectMake(0, kNavHeight, PYScreenW, KHEIGHT-kNavHeight);
...
- (UILabel *)searchHistoryHeader
{
    if (!_searchHistoryHeader) {
        ...
        _searchHistoryHeader.backgroundColor = [UIColor redColor];
...
- (UIView *)searchHistoryView
{
    if (!_searchHistoryView) {
        ...
        searchHistoryView.backgroundColor = [UIColor orangeColor];
...
//    self.navigationController.navigationBar.backIndicatorImage = nil;
    ...
    [cancleButton setTitleColor:PYSEARCH_COLOR(138, 138, 138) forState:0];
...
//    self.searchHistoriesCachePath = PYSEARCH_SEARCH_HISTORY_CACHE_PATH;
    ...
    [searchBar setImage:[UIImage imageNamed:@"搜索gray"] forSearchBarIcon:UISearchBarIconSearch state:UIControlStateNormal];
...
- (void)setSearchApiSuggestions:(NSArray *)searchApiSuggestions{
...
- (void)tagDidCLick:(UITapGestureRecognizer *)gr
{
    ...
    self.lastSearchStr = label.text;
    
     _searchSuggestionView = (UITableView *)self.searchSuggestionVC.view;
    // popular search tagLabel's tag is 1, search history tagLabel's tag is 0.
    if (1 == label.tag) {
...
//    if (self.didSearchBlock) self.didSearchBlock(self, nil, _searchBar.text);
//    [self saveSearchCacheAndRefreshView];
//    NSMutableArray *history = [NSMutableArray arrayWithArray:[NSKeyedUnarchiver unarchiveObjectWithFile:self.searchHistoriesCachePath]];
//    self.hotSearches = history;
//    self.searchSuggestionVC.view.hidden = self.searchSuggestionHidden || !_searchBar.text.length || ![self.searchSuggestionVC.tableView numberOfRowsInSection:0];
...
//        searchSuggestionView.hidden = self.searchSuggestionHidden || !self.searchBar.text.length || 0 == numberOfRow;
        searchSuggestionView.hidden = self.searchSuggestionHidden || !self.searchBar.text.length;
...
#pragma mark - 这里由于坑货产品要求 原生"-搜索框-"已无法满足 自定义之
- (void)setupSearchNavView{
    UIView *backView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, KWIDTH, kNavHeight)];
    backView.backgroundColor = [UIColor whiteColor];
    [self.view addSubview:backView];
    
    UIView *grayView = [[UIView alloc]initWithFrame:CGRectMake(15, kStatusBarHeight+4, KWIDTH-57-15, 36)];
    grayView.backgroundColor = UIColorFromRGB(0xF5F5F5);
    grayView.layer.cornerRadius = 4;
    [backView addSubview:grayView];
    
    UITextField *tf = [[UITextField alloc]initWithFrame:CGRectMake(37, 0, grayView.width-37, 36)];
    tf.backgroundColor = UIColorFromRGB(0xF5F5F5);
    ...
    [tf setValue:UIColorFromRGB(0x8A8A8A) forKeyPath:@"_placeholderLabel.textColor"];
    [tf addTarget:self action:@selector(searchTestInput:) forControlEvents:UIControlEventEditingChanged];
    [grayView addSubview:tf];
    ...
    UIImageView *searchIcon = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"PYSearchImage"]];
    searchIcon.frame = CGRectMake(10, 9.2, 17.6, 17.6);
    searchIcon.contentMode = UIViewContentModeScaleAspectFit;
    [grayView addSubview:searchIcon];
    
    UIButton *zzcancelBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    zzcancelBtn.frame = CGRectMake(grayView.right+10, 0, 32, 22);
    zzcancelBtn.centerY = grayView.centerY;
    [zzcancelBtn setTitle:@"取消" forState:0];
    [zzcancelBtn setTitleColor:UIColorFromRGB(0x38CCB3) forState:0];
    zzcancelBtn.titleLabel.font = PFFONT(16);
    [zzcancelBtn addTarget:self action:@selector(tapZZCancel) forControlEvents:UIControlEventTouchUpInside];
    [backView addSubview:zzcancelBtn];
    
}
- (void)searchTestInput:(UITextField *)tf{
    ...
    [self.view bringSubviewToFront:self.searchSuggestionVC.view];
    if ([self.delegate respondsToSelector:@selector(searchViewController:searchTextDidChange:searchText:)]) {
        [self.delegate searchViewController:self searchTextDidChange:nil searchText:searchText];//这里传nil 对外无影响
    }
}
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField{
    if (PYSearchResultShowModeEmbed == self.searchResultShowMode) {
        self.searchResultController.view.hidden = 0 == _searchBar.text.length || !self.showSearchResultWhenSearchBarRefocused;
        self.searchSuggestionVC.view.hidden = self.searchSuggestionHidden || !_searchBar.text.length || ![self.searchSuggestionVC.tableView numberOfRowsInSection:0];
        if (self.searchSuggestionVC.view.hidden) {
            self.searchSuggestions = nil;
        }
        self.baseSearchTableView.hidden = _searchBar.text.length && !self.searchSuggestionHidden && ![self.searchSuggestionVC.tableView numberOfRowsInSection:0];
    }
    [self setSearchSuggestions:self.searchSuggestions];
    return YES;
}
...
//    [textField resignFirstResponder];//取消第一响应者
    if ([self.delegate respondsToSelector:@selector(searchViewController:didSearchWithSearchBar:searchText:)]) {
        [self.delegate searchViewController:self didSearchWithSearchBar:nil searchText:_searchBar.text];
        [self saveSearchCacheAndRefreshView];
        return YES;
    }
    if (self.didSearchBlock) self.didSearchBlock(self, nil, _searchBar.text);
