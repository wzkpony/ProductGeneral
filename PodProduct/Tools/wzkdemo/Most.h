...
+(NSString *)loadPathWithName:(NSString *)filename;//读文件的时候的文件路径（现在工程里找，要是没有在沙盒里找，要是都没有就会返回nil）
...
+(void)getDBForPathCopy:(NSString*)name;//将工程文件拷贝到沙盒
...
+(NSArray* )loadFileForArrayWithPath:(NSString*)path;//本地数组
